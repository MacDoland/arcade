...
/*  */
	var vx = 0, vy = 0, maxX, maxY,
	originX = 0, originY = 0, dx_control = 0, dy_control = 0,
	down = 0, right = 0, minMovement = 15, fastDistance = minMovement*2,
	speed_control = 2, fastSpeed = 5,
	control;
...
/* TOUCH EVENTS */
	document.addEventListener('touchstart', function(e){
		...
		originX = touchobj.pageX;
		originY = touchobj.pageY;
		revealController();
		e.preventDefault();
	}, false);
	document.addEventListener('touchmove', function(e){
		var touchobj = e.changedTouches[0];
		dx_control = parseInt(e.changedTouches[0].clientX) - originX; 
		dy_control = parseInt(e.changedTouches[0].clientY) - originY;
		movementCalculation();
		e.preventDefault();
	}, false);
	document.addEventListener('touchend', function(e){
		realClassUtils.addClass('control', 'off');
		dx_control = dy_control = vx = vy = 0;
		e.preventDefault();
	}, false);

/* TOUCH CONTROLLER METHODS */
	function movementCalculation(){
		if(Math.abs(dx_control) > minMovement || Math.abs(dy_control) > minMovement){
			right = Math.sign(dx_control);
			down = Math.sign(dy_control);
			dx_control = Math.abs(dx_control);
			dy_control = Math.abs(dy_control);
		}
	}

	function revealController(){
		realClassUtils.removeClass('control', 'off');
		control.style.left = originX + 'px';
		control.style.top = originY + 'px';
	}
...
/* UPDATE */
	...
	function updateTouchController(){
		if(dx_control > minMovement){
			rightDown = right > 0 ? true : false;
			leftDown = right < 0 ? true : false;
		}
		if(dy_control > minMovement){
			downDown = down > 0 ? true : false;
			upDown = down < 0 ? true : false;
		}
	}
